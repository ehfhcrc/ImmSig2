...
labkey.url.base <- "https://test.immunespace.org" # need for features to load correctly
...
for(sdy in studies){
  ...
  # Annotation for Discovery Cohort and SDY67 is particular to time the microarrays were read.
  # Using the default / BioConductor Anno from ImmuneSpace causes slightly different
  # results and therefore is not used for those studies.
  annotation <- ifelse(sdy == "SDY80",
                       "default",
                       "ImmSig")
  
  outputType <- ifelse(sdy == "SDY67",
                       "normalized",
                       "raw")
  
  # Need raw, non-normalized data for Studies 212, 63, 404 to match manuscript
  # Studies 400 and 80 can use GEO's normalized series as "raw"
  # Annotation is "original", meaning the gene symbol to probe mapping from
  # when the matrix was created in ImmuneSpace.
  es <- con$getGEMatrix(mats, outputType = outputType, annotation = annotation)
  
  # SDY212 has special requirements:
  # 1. probe with NAs must be taken out before normalization
  # 2. dup biosamples must be taken out of eset prior to sub-id mapping
  if(sdy == "SDY212"){
    # expr matrix
    em <- exprs(es)
