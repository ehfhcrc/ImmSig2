...
for(sdy in studies){
  ...
  if(sdy == "SDY67"){
    ...
    es <- con$getGEMatrix(mats, outputType = "raw", annotation = "original") # need raw
    
    # SDY212 has a duplicate biosample that is taken out after normalization
    if(sdy == "SDY212"){
      # expr matrix
      em <- exprs(es)
    ...
    es <- con$EMNames(es) # map biosample ids to subids
    em <- exprs(es) # coming in as a matrix 
    colnames(em) <- gsub(paste0(".", gsub("SDY", "", sdy)),"",colnames(em)) # fix from "SUB123123.212_d0" 
  ...
  # Remove two subs not found in original file
  if(sdy == "SDY212"){ 
